    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
	  ...
	  v *= 0.44;
          ...
          for (uint i=0; i <ptsx.size(); i++){
            ...
            // Xm = xp + (xc * cos(teta) - yc * sin(teta))
            // Ym = yp + (xc * sin(teta) - yc * cos(teta))
            //xc,yc - map coordinates
            std::cout << "points conversion: " << ptsx[i] << " " << ptsy[i] << "=>";
            // we assume that car is at 0,0
          ...
          std::cout << "\n -------------------- \n";
          std::cout << "car position x = " << px << "  y = " << py << std::endl;
          std::cout << "car angle = " << psi << " speed = " << v << std::endl;

          // convert to eigen
          double* ptr_ptsx = &ptsx[0];
          double* ptr_ptsy = &ptsy[0];

          Eigen::Map<Eigen::VectorXd> ptsx_conv(ptr_ptsx,6);
          Eigen::Map<Eigen::VectorXd> ptsy_conv(ptr_ptsy,6);
		...
		//v += a*latency;
          ...
          mpc.step++;
          std::cout << "step" << mpc.step << std::endl;

          //if (mpc.step > 940) getchar();
          // Latency
